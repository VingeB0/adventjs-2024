...
function calculatePrice(ornaments) {
    ...
    for (let i = 0; i < ornaments.length; i++) {
        const currentPrice = ornamentsPrice[ornaments[i]];
        const nextPrice = ornamentsPrice[ornaments[i + 1]];

        if (currentPrice === undefined) return undefined;

        // Subtract if the next ornament has a higher price, otherwise add
