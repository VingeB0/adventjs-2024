...
function decodeFilename(filename) {
    let splittedExtension = filename.split('.');
    if(splittedExtension.length > 2) splittedExtension.length = 2;
    splittedExtension = splittedExtension.join('.')
    let splittedPrefix = splittedExtension.split('_');
    return splittedPrefix.slice(1).join('_');
}